...
Win::Win(enum SCENES ID, sf::RenderWindow *window, sf::Time *elapsedTime)
{
    setID(ID);
    _window = window;
    _elapsedTime = elapsedTime;
    _entities[std::string("000_parralax1")] = new ParralaxSprite(PARRALAX_SPRITE, _window, _elapsedTime, new Position(0, 0, "position"), new Sprite("../assets/win.jpg", "sprite"), new Scale(2.2, 2.2, "scale"), new Speed(0, 0, "speed"));
...
std::map<std::string, GE::IEntity *> Win::getEntities()
{
    return _entities;
}

void Win::switchButtonDown()
{
    for (std::map<std::string, GE::IEntity *>::const_iterator it = _entities.begin(); it != _entities.end(); ++it)
    {
...
fptr Win::eventListener(sf::Event event)
{
    switch (event.type)
    {
...
void Win::addEntity(enum ENTITY_TYPE type, sf::Vector2f position)
{
    (void) type;
    (void) position;
    return;
